...
/// Macro for the rounding method for the tick operations in the AETimer.
/// 
/// Since when we work with the tick amount in the AETimer, we may (or will) 
/// encounter floating point numbers. So we need to find a way to deal with them. 
/// And that's what this is for -- it defines the rounding method for such cases.
/// 
/// List of acceptable values:
/// * **0** -- round down the resulting floating point
/// * **1** -- round the resulting point using standard math: from 0 to 0.499.. round down; from 0.5 and up -- round up;
/// * **2** -- round up the resulting floating point
/// 
/// @note Adjust for your need
/// @attention Passing the invalid value will give you a preprocessing-time error
#define AETM_TICK_ROUNDING_METHOD 1
